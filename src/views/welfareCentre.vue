<template>
  <div class="p-welfareCentre">
    ...
    <div class="p-welfareCentre-sign">
      ...
      <div class="-sign-top">
        <div class="-sign-width">
          ...
          <div class="-time">
            <span class="-num">{{ learnTime }}</span>分钟
          </div>
        </div>
        <div class="-sign-width">
      ...
      <div class="-sign-bottom">
        <div class="-sign-bottom-wrap" :class="{ '-sign-bottom-wrap-two': isTimeToCollectThree }">
          <div class="-wrap-with" v-if="dataDetail.myCredit.getCreditLearnThree && !isTimeToCollectThree">
            <div class="-one">+20</div>
          </div>

          <img class="-wrap-with -animation" v-if="isTimeToCollectThree && !dataDetail.myCredit.getCreditLearnThree"
        ...
        <div class="-sign-bottom-wrap" :class="{ '-sign-bottom-wrap-two': isTimeToCollectFive }">
          <div class="-wrap-with" v-if="dataDetail.myCredit.getCreditLearnFive && !isTimeToCollectFive ">
            <img class="-center-img" src="../assets/image/fuli/backgroud-3.png"/>
            <div class="-one -center-num">+30</div>
          </div>

          <img class="-wrap-with -animation" v-if="isTimeToCollectFive && !dataDetail.myCredit.getCreditLearnFive"
               src="../assets/image/fuli/button-coin-pre.png"
        ...
        <div class="-sign-bottom-wrap" :class="{ '-sign-bottom-wrap-two': isTimeToCollectTen }">
          <div class="-wrap-with" v-if="dataDetail.myCredit.getCreditLearnTen && !isTimeToCollectTen">
            <div class="-one">+50</div>
          </div>
          <img class="-wrap-with -animation" v-if="isTimeToCollectTen && !dataDetail.myCredit.getCreditLearnTen"
               src="../assets/image/fuli/button-coin-pre.png" @click="toGetCredit(10, 50)"/>
          <div class="-wrap-with" v-if="!dataDetail.myCredit.getCreditLearnTen && !isTimeToCollectTen"
               @click="showTip(10)">
            <img class="-three" src="../assets/image/fuli/button-coin-def.png"/>
          </div>
        ...
        <div class="-sign-bottom-solid" v-if="learnTime!=0" :style="{width: widthData}"></div>
      </div>
    </div>
    ...
    <van-popup v-model="isShowService" :close-on-click-overlay="false" class="p-welfareCentre-popupService">
      <div class="-body">
        <div class="-title">
        ...
        <div class="-tip">
          <img class="-safe-img" src="../assets/image/duihuan/gz-icon-safe.png"/>安全认证
        </div>
      </div>
    ...
    methods: {
      ...
      welfareDetail() {
        ...
        this.$api.welfare.welfareInfo().then(({data}) => {
          ...
          this.widthData = `${((nowTime > TenTime ? TenTime : nowTime) / TenTime) * 240}px`

          if (nowTime >= threeTime && !this.dataDetail.myCredit.getCreditLearnThree) {
            this.isTimeToCollectThree = true;
          } else {
            this.isTimeToCollectThree = false;
          }

          if (nowTime >= FiveTime && !this.dataDetail.myCredit.getCreditLearnFive) {
